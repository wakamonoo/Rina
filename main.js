...
const deepLink = (id) => {
  ...
  if (isIOS) return `youtube://www.youtube.com/watch?v=${id}`;
  /* desktop */
...
async function autoplay(q) {
  try {
    ...
    window.location.href = deepLink(id); // current tab on mobile → launches app
    if (!(isAndroid || isIOS)) status.textContent = "Opened in new tab.";
  } catch {
...
async function listResults(q) {
  try {
    ...
    data.items.forEach((it, i) => {
      ...
      a.href = deepLink(id);
      a.target = isAndroid || isIOS ? "_self" : "_blank";
      a.className =
